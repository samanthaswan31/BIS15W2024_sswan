...
```
Changing it- replace scientist in the hotsprings data frame with the same data but as a factor
```{r}
...
Filter for little fish less than or equal to 100
```{r}
...
```{r}
test <- c(mammals$order, mammals$family, mammals$Genus)
table(test)
```
