...
```{r}
scientist <- name
hsprings_version_two <- data.frame(temp_C, scientist, spring)
hsprings_version_two
```
...
My method:
```{r}
hsprings_version_two$depth <- c(4.15, 4.13, 4.12, 3.21, 3.23, 3.20, 5.67, 5.65, 5.66)
hsprings_version_two
```

4. Calculate the mean temperature of all of the temperature measurements.
Way done in class:
hummingbirds](https://www.audubon.org/field-guide/bird/ruby-throated-hummingbird).  
