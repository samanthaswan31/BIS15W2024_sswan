...
Class strategy: pull out everything with a number, leave all NAs behind
```{r}
penguins %>%
...
```{r}
penguins %>%
  summarize(across(contains("mm"), \(x) mean(x, na.rm = TRUE))) #use the slash x to correct the error
```
