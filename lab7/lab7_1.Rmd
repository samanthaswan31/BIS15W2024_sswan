...
Must specify numeric value of interest
```{r}
...
Total n gives total number of observations/number of rows
```{r}
msleep %>%
  filter(bodywt > 200) %>%
  summarize(mean_sleep_lg = mean(sleep_total),
...
```{r}
msleep %>%
  summarize(n_genera = n_distinct(genus)) #counts number of distinct genera in msleep
```
...
```{r}
msleep %>%
  summarize(min_sleeptotal_mammals = min(sleep_total),
            max_sleeptotal_mammals = max(sleep_total),
            mean_sleeptotal_mammals = mean(sleep_total),
            total=n())
```
...
msleep %>%
  group_by(order) %>%
  summarize(mean_bw_order = mean(brainwt))
```
...
msleep %>%
  group_by(order) %>%
  summarize(mean_bw_order = mean(brainwt, na.rm=T))
```
