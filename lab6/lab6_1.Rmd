...
```
Clean_names is attached via a pipe, increases efficiency (via janitor package)
```{r}
...
New variable always added to the end of the data frame
```{r}
fish %>% 
...
ifelse allows the replacement of the -999.00 values with NA
```{r}
mammals %>% 
  select(genus, species, newborn) %>%
