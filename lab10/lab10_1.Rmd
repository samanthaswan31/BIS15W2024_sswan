...
Don't have to put the x= as x can only be one variable (doing counts in that variable)
```{r}
homerange %>% 
...
Specified y first, flips the plot
Coord_flip also works to flip the plot
```{r}
homerange %>% 
  filter(family=="salmonidae") %>%
  select(common.name, log10.mass) %>% 
  ggplot(aes(y=common.name, x=log10.mass))+ #notice the switch in x and y or geom_bar(stat="identity")
  geom_col()+
  coord_flip()
```
...
Coord flip keeps the names from "tangling" illegibly 
```{r}
homerange_mammals %>%
  filter(class=="mammalia") %>%
  top_n(-10, log10.mass) %>%
