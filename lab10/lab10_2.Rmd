...
Straight count use bar- generate numerical summary first
Class method:
```{r}
penguins %>%
...
penguins %>%
  filter(sex!="NA") %>%
  group_by(sex) %>%
  summarize(avg_bill_length_mean=mean(bill_length_mm)) %>%
  ggplot(aes(x=sex, y=avg_bill_length_mean))+
  geom_col()
```
...
No group by= boxplot shows the same thing
```{r}
homerange %>% 
