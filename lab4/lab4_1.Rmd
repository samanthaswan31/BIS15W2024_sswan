...
Fish not in quotes as it is an object, "" required for variables
```{r}
...
Order of product is related to the order in which the variables are added
```{r}
fihs_subset <- select(fish, "fish_id", "length")
```
...
glimpse(fish)
```
OR select_if(fish, is.numeric()) for is numeric, ! means is not
```{r}
...
summary(mammals)
```
Wean mass minimum is -999 which is incorrect
```{r}
class(mammals$`max. life`)
```
...
```{r}
mammals_new <- rename(mammals, genus = "Genus", wean_mass = "wean mass", max_life = "max. life", litters_year = "litters/year", litter_size = "litter size")
```
...
```{r}
mammals_excluded <- select(mammals_new, -"order", -"family")
```

7. Select the columns that include "mass" as part of the name.  
```{r}
mammals_mass <- select(mammals_new, contains("mass"))
mammals_mass
```
