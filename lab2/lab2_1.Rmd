...
x 
```
Bad annotation, want to use the arrow gets rather than equal as gets confusing later
```{r}
z=30 #do not use
```
...
my_experiment
```
Example: wanting to try another way
```{r}
treatment+control
```
...
```
Alternative method to get the correct answer is to use c to link the objects together- recognize as 3 seperate objects
```{r}
...
```
My integer gets my numeric (translation of code) so will say TRUE as it has been changed
```{r}
my_integer <- 
