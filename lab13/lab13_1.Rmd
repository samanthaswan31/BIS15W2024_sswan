...
```{r}
butte <- spider_list[["Butte"]] #can now directly use the county name, can store as object
```
...
`inner_join`  only matches the observations for which all of the columns are present, automatically drops customer 3 and 4
...
`full_join` Regardless of missing data, oftentimes what is used
```{r}
...
Left- have one data frame (all) and want to add new data on to the end (left)
```{r}
