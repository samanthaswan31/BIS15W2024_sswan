...
elephants %>%
  ...
  geom_point()+
  geom_smooth(method = lm, se = F) #lm is linear model, do not include standard error for se is false
```
...
elephants %>% 
  ...
  labs(title="Elephant Age vs. Height",
       x="Age (years)",
       y="Height (cm)")+
  theme(plot.title = element_text(size = rel(1.5), hjust = 0.5)) #rel is title is 1.5x larger relative to the axis titles, hjust is horizontal adjustment of title (how centered it is)
```
