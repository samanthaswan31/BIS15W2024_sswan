...
```r
amniota %>% 
  replace_with_na(replace = list(gestation_d= -999.0, 
                                 subspecies= -999, 
...
```r
miss_var_summary(amniota_replaced)
```
...
Biologically- mammals don't lay eggs so it being 100% NAs makes sense and then the respective numbers for birds versus reptiles is dramatically different- far higher for reptiles than birds. This changes my interpreation of the NA as it may correspond to not found.
...
I don't think they reprsent true NAs- the distribution should be more equal. Perhaps it has to do with preferred habitats or mode of transportation versus what the animal is physically capable of?
