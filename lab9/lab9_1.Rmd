...
(1) each variable has its own column?  
 Each variable has its own row rather than column
 
(2) each observation has its own row?  
Drug = observation, in this data set up each has it's own column rather than row or column with all drug
Drug needs to be own column
...
Untidy because each observation has its own column with values
Should be each observation should be in column with "income" at the title
...
Useful when data has been recorded sequentially (like across time or DNA nucleotides)
```{r}
billboard2 <- 
  billboard %>% 
...
qpcr_untidy %>% 
  pivot_longer(
    exp1_rep1:exp3_rep3, #: gives range
    names_to= c("experiment", "replicate"),
    names_sep="_", #seperate by the underscore
