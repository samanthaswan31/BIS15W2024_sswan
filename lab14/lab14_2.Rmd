  ...
  dashboardBody(
    fluidRow(
      box(width=3,
          ...
          selectInput("x", "Select Population", choices = unique(wolves$pop)), hr() #gives help with multiple observations
...
server <- function(input, output, session) {
  ...
  output$plot <- renderPlot({
    wolves %>%
      filter(sex!="NA") %>% 
      filter(pop==input$x) %>% #population is the variable so filter is the reactive part
  ggplot(aes(x=sex, fill=sex))+
  geom_bar()
  })
...
ui <- fluidPage(    
  ...
  #shinythemes::themeSelector(),  # <--- Add this somewhere in the UI, gives theme options for selection
  theme = shinytheme("superhero"), #can set the theme here
