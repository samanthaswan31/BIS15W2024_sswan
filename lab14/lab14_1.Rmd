...
```{r}
penguins %>% 
  ggplot(aes(x=bill_length_mm, y=body_mass_g))+ #Before building any shinyApp, build plot at least once in advance
  geom_point()
```
...
shinyApp(ui, server)
```
Trouble shooting- start by removing everything one by one (start with aesthetics)
